...
import { CombatState, Stat, InventoryItem } from '../types';
    ...
    if (activeCombat.victory) {
        return (
            <div className="absolute inset-0 bg-black/95 flex items-center justify-center p-8">
                <div className="w-full h-full max-w-7xl border-8 border-double border-red-500/50 flex flex-col p-6 items-center justify-center">
                    <h2 className="text-6xl text-yellow-400 mb-8 animate-pulse">VITTORIA</h2>
                    <div className="text-3xl text-green-200 space-y-4 mb-12 text-center">
                       <p>{activeCombat.log[activeCombat.log.length - 2]?.text}</p>
                       <p className="italic text-gray-400">{activeCombat.log[activeCombat.log.length - 1]?.text}</p>
                    </div>
                ...
                <div className="flex-grow flex space-x-6 overflow-hidden">
                    ...
                    <div className="w-2/5 h-full flex flex-col">
                        <div className="flex-grow flex flex-col border-2 border-green-400/30 p-4">
                             {/* Status */}
                            <div className="border-b-2 border-red-500/30 pb-2 mb-2 text-3xl">
                            ...
                            <div className="grid grid-cols-1 gap-y-2 text-4xl mt-auto">
                                {isItemMenuOpen ? (
                                     usableItems.map((item, index) => {
                                        ...
                                        return (
                                            <div key={item.itemId + index} className={`${isSelected ? 'bg-green-400 text-black' : ''} pl-2`}>
